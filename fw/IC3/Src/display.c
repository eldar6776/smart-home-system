...
/* Private Variable ----------------------------------------------------------*/
char oskin[6];
uint32_t rtc_tmr=0,out1_tmr=0,refresh_tmr=0;
uint32_t dispfl,lightauto,onofftmr,keypadtmr=0,keypadtout=0;
uint32_t menu_tmr=0,clean_tmr=0,clrtmr=0,guitmr=0,scrnsvr_tmr,rtctmr;
uint8_t btn_ok_state,btnset,pswb=4,low_bcklght,high_bcklght,light_ldr;
uint8_t screen,ctrl1,ctrl2,ctrl3,disp_rot,menu_screen=0,last_state=0;
uint8_t scrnsvr_ena_hour,scrnsvr_dis_hour,scrnsvr_clk_clr,scrnsvr_tout;
uint8_t btninc,_btninc,btndec,_btndec,menu_thst=0,menu_lc=0,old_min=60;
uint8_t menu_dim=0,menu_rel123=0,menu_out1=0,thsta=0,oskb,kyret=0,fwmsg=2;
uint8_t menu_alarm=0,menu_light=0,menu_clean=0;
/* Private Macro -------------------------------------------------------------*/
...
void DISPService(void){
    ...
    switch(screen){
        ...
        case 1:
        {
            ...
            GUI_Clear();
            GUI_SetPenSize(9);
            if      ( ctrl1 && !Ctrl1.Main1.value) GUI_ClearRect( 90,100,160,160), Ctrl1.Main1.value=1, GUI_SetColor(GUI_GREEN),GUI_DrawEllipse(150,136,50,50), Light1On();
            else if ( ctrl1 &&  Ctrl1.Main1.value) GUI_ClearRect( 90,100,160,160), Ctrl1.Main1.value=0, GUI_SetColor(GUI_RED),  GUI_DrawEllipse(150,136,50,50), Light1Off();
            else if (!ctrl1 && !Ctrl1.Main1.value) GUI_ClearRect( 90,100,160,160),                      GUI_SetColor(GUI_RED),  GUI_DrawEllipse(150,136,50,50), Light1Off();
            else if (!ctrl1 &&  Ctrl1.Main1.value) GUI_ClearRect( 90,100,160,160),                      GUI_SetColor(GUI_GREEN),GUI_DrawEllipse(150,136,50,50), Light1On();
            if      ( ctrl2 && !Ctrl1.Main2.value) GUI_ClearRect(330,100,390,160), Ctrl1.Main2.value=1, GUI_SetColor(GUI_GREEN),GUI_DrawEllipse(330,136,50,50), Light2On();
            else if ( ctrl2 &&  Ctrl1.Main2.value) GUI_ClearRect(330,100,390,160), Ctrl1.Main2.value=0, GUI_SetColor(GUI_RED),  GUI_DrawEllipse(330,136,50,50), Light2Off();
            else if (!ctrl2 && !Ctrl1.Main2.value) GUI_ClearRect(330,100,390,160),                      GUI_SetColor(GUI_RED),  GUI_DrawEllipse(330,136,50,50), Light2Off();
            else if (!ctrl2 &&  Ctrl1.Main2.value) GUI_ClearRect(330,100,390,160),                      GUI_SetColor(GUI_GREEN),GUI_DrawEllipse(330,136,50,50), Light2On();
        ...
        case 8:
        {
            ...
            Ctrl1.Main2.index   = SPINBOX_GetValue(hBIN_MAIN2);
...
void PID_Hook(GUI_PID_STATE * pTS){
    ...
    if (pTS->Pressed == 1){
        ...
        if ((pTS->y >  60) && (pTS->y < 200) && (screen == 3) && (menu_alarm > 1)){
        ...
        }else if((pTS->x > 380) && (pTS->y <  80) && (screen < 5)){
            click = 1;
            ...
            click  = 1;
            btndec = 1;
        }else if((pTS->y > 60) && (screen == 0)){
            if (!onofftmr){
                ctrl1 = 0;
                ctrl2 = 0;
                if(pTS->x < 240)ctrl1 = 1;
                else            ctrl2 = 1;
                screen = 1; 
                click  = 1;
            }
...
static void DISPDateTime(void){
    ...
    if (IsScrnsvrActiv() && IsScrnsvrEnabled() && IsScrnsvrClkActiv()){
        ...
        if      (rtcdt.Month == 0x1) memcpy(&dbuf[12],". January ",10);		
        else if (rtcdt.Month == 0x2) memcpy(&dbuf[12],". February",10);
        else if (rtcdt.Month == 0x3) memcpy(&dbuf[12],".  March  ",10);
        else if (rtcdt.Month == 0x4) memcpy(&dbuf[12],".  April  ",10);
        else if (rtcdt.Month == 0x5) memcpy(&dbuf[12],".   May   ",10);
        else if (rtcdt.Month == 0x6) memcpy(&dbuf[12],".   June  ",10);
        else if (rtcdt.Month == 0x7) memcpy(&dbuf[12],".   July  ",10);
        else if (rtcdt.Month == 0x8) memcpy(&dbuf[12],". August  ",10);
        else if (rtcdt.Month == 0x9) memcpy(&dbuf[12],".September",10);
...
static void DISPInitSet2Scrn(void){
    ...
    hBIN_MAIN2 = SPINBOX_CreateEx(10, 90, 90, 30, 0, WM_CF_SHOW, ID_BIN_MAIN2, 0, 24);
    SPINBOX_SetEdge(hBIN_MAIN2, SPINBOX_EDGE_CENTER);
    SPINBOX_SetValue(hBIN_MAIN2, Ctrl1.Main2.index);    
    hBIN_ALARM1 = SPINBOX_CreateEx(10,130, 90, 30, 0, WM_CF_SHOW, ID_BIN_ALARM1, 0, 24);
    SPINBOX_SetEdge(hBIN_ALARM1, SPINBOX_EDGE_CENTER);
    SPINBOX_SetValue(hBIN_ALARM1, Ctrl1.Alarm1.index);    
    hBIN_ALARM2 = SPINBOX_CreateEx(10,170, 90, 30, 0, WM_CF_SHOW, ID_BIN_ALARM2, 0, 24);
    SPINBOX_SetEdge(hBIN_ALARM2, SPINBOX_EDGE_CENTER);
    SPINBOX_SetValue(hBIN_ALARM2, Ctrl1.Alarm2.index);    
    hBIN_ALARM3 = SPINBOX_CreateEx(10, 210, 90, 30, 0, WM_CF_SHOW, ID_BIN_ALARM3, 0, 24);
    ...
    GUI_GotoXY(110, 54);
    GUI_DispString("MAIN SW1");
    GUI_GotoXY(110, 66);
    GUI_DispString("RELAY1");    
    GUI_GotoXY(110, 94);
    GUI_DispString("MAIN SW2");
    GUI_GotoXY(110, 106);
    GUI_DispString("RELAY1");    
    GUI_GotoXY(110, 134);
...
static void SaveController(CtrlTypeDef* lc, uint16_t addr){
    ...
    buf[1] = lc->Main2.index;
    buf[2] = lc->Alarm1.index;
    buf[3] = lc->Alarm2.index;
    buf[4] = lc->Alarm3.index;
    EE_WriteBuffer(buf, addr, 5);
}

static void ReadController(CtrlTypeDef* lc, uint16_t addr){
    uint8_t buf[5];
    EE_ReadBuffer(buf, addr, 5);
    lc->Main1.index     = buf[0];
    lc->Main2.index     = buf[1];
    lc->Alarm1.index    = buf[2];
    lc->Alarm2.index    = buf[3];
    lc->Alarm3.index    = buf[4];
}
