...
/* Private Define ------------------------------------------------------------*/
#define KEY_PAUSE                       20      // 
#define LADDER_RUNTIME                  45000   // ladder motor up/down run time
#define LADDER_ONOFF_DELAY              1000    // min. delay between roller up and down
...
char oskin[6];
uint32_t rtc_tmr, out1_tmr, refresh_tmr, laddertmr;
...
uint8_t screen, ctrl1, ctrl2, ctrl3, ctrl4, disp_rot, menu_screen, last_state;
uint8_t menu_keypad, menu_ladder, menu_clean, fwmsg=2, kyret, old_min=60;
...
void DISPService(void){
    ...
    switch (screen){
        ...
        case 1:
        {
            ...
            GUI_MULTIBUF_BeginEx(1);
            GUI_ClearRect(400,0,460,80);
            GUI_ClearRect( 90,100,160,160);
            GUI_SetPenSize(9);
            if      ( ctrl1 && !Ctrl1.Main1.value) GUI_SetColor(GUI_GREEN), Light1On(), Ctrl1.Main1.value=1;
            else if ( ctrl1 &&  Ctrl1.Main1.value) GUI_SetColor(GUI_RED),   Light1Off(),Ctrl1.Main1.value=0;
            else if (!ctrl1 && !Ctrl1.Main1.value) GUI_SetColor(GUI_RED),   Light1Off();
            else if (!ctrl1 &&  Ctrl1.Main1.value) GUI_SetColor(GUI_GREEN), Light1On();
            if      ( ctrl2 && !Ctrl1.Main2.value) Ctrl1.Main2.value=1,     Light2On();
            else if ( ctrl2 &&  Ctrl1.Main2.value) Ctrl1.Main2.value=0,     Light2Off();
            GUI_DrawLine(400,20,450,20);
            GUI_DrawLine(400,40,450,40);
            GUI_DrawLine(400,60,450,60);
            GUI_DrawEllipse(150,136,50,50),
            GUI_SetColor(GUI_BLUE);
            GUI_DrawEllipse(330,136,50,50);
            GUI_MULTIBUF_EndEx(1);
            menu_screen = 0;
            menu_ladder = 0;
            menu_keypad = 0;
        ...
        case 2:
        {
            if (menu_screen == 0){
                ...
                GUI_DrawBitmap(&bmLADDER,   80, 23);
                GUI_DrawBitmap(&bmKUCA,    320, 23);
        ...
        case 4:
        {
            if(menu_ladder == 0){
                ...
                GUI_DrawLine(400,40,450,40);
                GUI_DrawLine(400,60,450,60);
                GUI_DrawBitmap(&bmLADDER_BIG, 220, 80);
                GUI_SetColor(GUI_RED);
                if (!IsLadderUpActive()) GUI_DrawPolygon(aLadderUp,3,60,40);
                else GUI_FillPolygon(aLadderUp,3,60,40),    laddertmr = HAL_GetTick();
                GUI_SetColor(GUI_BLUE);
                if (!IsLadderDownActive()) GUI_DrawPolygon(aLadderDown,3,60,180);
                else GUI_FillPolygon(aLadderDown,3,60,180), laddertmr = HAL_GetTick();
                GUI_MULTIBUF_EndEx(1);
                menu_screen = 0;
                menu_keypad = 0;
            } else if (menu_ladder == 1){                
                if (ctrl3){
                    ++menu_ladder;
                    LadderDownOff();
                    GUI_MULTIBUF_BeginEx(1);
                    GUI_ClearRect(0,0,220,272);
                    GUI_SetColor(GUI_BLUE);
                    GUI_DrawPolygon(aLadderDown,3,60,180);
                    GUI_SetColor(GUI_RED);
                    if (!IsLadderUpActive()){
                        GUI_FillPolygon(aLadderUp,3,60,40);
                        HAL_Delay(LADDER_ONOFF_DELAY);
                        laddertmr = HAL_GetTick();
                        LadderUpOn();
                    } else if(IsLadderUpActive()){
                        GUI_DrawPolygon(aLadderUp,3,60,40);
                        LadderUpOff();
                        laddertmr = 0;                        
                    }
                    GUI_MULTIBUF_EndEx(1);
                    ...
                    GUI_DrawPolygon(aLadderUp,3,60,40);
                    GUI_SetColor(GUI_BLUE);
                    if (!IsLadderDownActive()){
                        GUI_FillPolygon(aLadderDown,3,60,180);
                        HAL_Delay(LADDER_ONOFF_DELAY);
                        laddertmr = HAL_GetTick();
                        LadderDownOn();
                    } else if (IsLadderDownActive()){
                        GUI_DrawPolygon(aLadderDown,3,60,180);
                        LadderDownOff();
                        laddertmr = 0;
                    }
            ...
            } else if ((menu_ladder == 2) && !ctrl3 && !ctrl4){
                menu_ladder = 1;
            }
    ...
    if (laddertmr){
        if (HAL_GetTick() - laddertmr >= LADDER_RUNTIME){
            laddertmr = 0;
            LadderUpOff();
            LadderDownOff();
            BuzzerOn();
...
void PID_Hook(GUI_PID_STATE * pTS){
    ...
    if (pTS->Pressed == 1){
        pTS->Layer = 1;
        if((pTS->x > 400) && (pTS->y < 60) && (screen < 5)){
            click = 1;
            if (screen == 2) screen = 6;
            else screen = 2;
        }else if((pTS->y > 60) && (screen == 0)){
            if (!onofftmr){
                ctrl1 = 0, ctrl2 = 0, click = 1, screen = 1;
            ...
            click = 1;
            screen = 3;
        }else if((pTS->x < 200) && (screen == 4)){
            ctrl3 = 0, ctrl4 = 0, click = 1;
            if (pTS->y < 136)     ctrl3 = 1;
            else                  ctrl4 = 1;
        }
        if (click){
            BuzzerOn();
