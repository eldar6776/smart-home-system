...
{
	TRANSPORT_HAL_DEBUG(PSTR("THA:SAD:ADDR=%u\n"), address);
...
uint8_t transportHALGetAddress(void)
{
	...
	TRANSPORT_HAL_DEBUG(PSTR("THA:GAD:ADDR=%u\n"), result);
	return result;
}
...
bool transportHALSanityCheck(void)
{
	...
	TRANSPORT_HAL_DEBUG(PSTR("THA:SAN:RES=%u\n"), result);
	return result;
}
...
bool transportHALReceive(MyMessage *inMsg, uint8_t *msgLength)
{
	...
	if (!tmp.isProtocolVersionValid()) {
		...
		TRANSPORT_HAL_DEBUG(PSTR("!THA:RCV:PVER=%u\n"),
	...
	*msgLength = tmp.getLength();
	...
	if (payloadLength != expectedMessageLength) {
		...
		TRANSPORT_HAL_DEBUG(PSTR("!THA:RCV:LEN=%u,EXP=%u\n"), payloadLength,
		                    expectedMessageLength); // invalid payload length
		return false;
	}
#endif
	TRANSPORT_HAL_DEBUG(PSTR("THA:RCV:MSG LEN=%u\n"), payloadLength);
	return true;
}
                      ...
                      const bool noACK)
{
	...
	TRANSPORT_HAL_DEBUG(PSTR("THA:SND:MSG LEN=%u,RES=%u\n"), finalLength, result);
	return result;
}
