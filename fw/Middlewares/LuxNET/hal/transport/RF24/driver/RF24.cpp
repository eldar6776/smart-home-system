...
uint8_t RF24_spi_txbuff[32+1]; //SPI transmit buffer (payload max 32 bytes + 1 byte for the command)
#endif
...
LOCAL void RF24_csn(const bool level)
{
	...
	(void)level;
#else
	//hwDigitalWrite(MY_RFCS_PORT, MY_RFCS_PIN, level);
	HAL_GPIO_WritePin(MY_RF24_CS_PORT, MY_RF24_CS_PIN, level == true ? GPIO_PIN_SET : GPIO_PIN_RESET);
#endif
}

LOCAL void RF24_ce(const bool level)
{
	HAL_GPIO_WritePin(MY_RF24_CE_PORT, MY_RF24_CE_PIN, level == true ? GPIO_PIN_SET : GPIO_PIN_RESET);
}


LOCAL uint8_t RF24_spiMultiByteTransfer(const uint8_t cmd, uint8_t *buf, uint8_t len, const bool readMode)
{
    uint8_t status;
	uint8_t *current = buf;
	RF24_csn(LOW);
	delayMicroseconds(10);
	HAL_SPI_TransmitReceive(&hspi1,(uint8_t*)&cmd, (uint8_t*)&status, 1, HAL_MAX_DELAY);
    while ( len-- ) {
		if (readMode) {
            HAL_SPI_Receive(&hspi1,(uint8_t*)&status, 1, HAL_MAX_DELAY);
			if (buf != NULL) {
				*current++ = status;
			}
		} else {
             HAL_SPI_Transmit(&hspi1, current++, 1, HAL_MAX_DELAY);
		}
	}
	RF24_csn(HIGH);
	delayMicroseconds(10);
...
{
	RF24_DEBUG(PSTR("RF24:OWP:RCPT=%u\n"), recipient); // open writing pipe
                            ...
                            const bool noACK)
{
	...
	RF24_DEBUG(PSTR("RF24:TXM:TO=%u,LEN=%u\n"), recipient, len); // send message
	// flush TX FIFO
	RF24_flushTX();
	if (noACK) {
...
LOCAL bool RF24_sanityCheck(void)
{
	...
	return (RF24_readByteRegister(RF24_REG_RF_SETUP) == RF24_RF_SETUP) && (RF24_readByteRegister(RF24_REG_RF_CH) == MY_RF24_CHANNEL);
}
...
{
	RF24_DEBUG(PSTR("RF24:INIT:PIN,CE=%u,CS=%u\n"), MY_RF24_CE_PIN, MY_RF24_CS_PIN);
	// Initialize pins & HW
#if (defined(MY_RF24_POWER_PORT)&& defined(MY_RF24_POWER_PIN))
	GPIO_SetPinMode(MY_RF24_POWER_PORT, MY_RF24_POWER_PIN, GPIO_MODE_OUTPUT_PP);
#endif
	RF24_powerUp();
#if defined(MY_RX_MESSAGE_BUFFER_FEATURE)
	GPIO_SetPinMode(MY_RF24_IRQ_PORT, MY_RF24_IRQ_PIN, GPIO_MODE_INPUT);
#endif
	GPIO_SetPinMode(MY_RF24_CE_PORT, MY_RF24_CE_PIN, GPIO_MODE_OUTPUT_PP);
#if !defined(__linux__)
	GPIO_SetPinMode(MY_RF24_CS_PORT, MY_RF24_CS_PIN, GPIO_MODE_OUTPUT_PP);
#endif
	RF24_ce(LOW);
	RF24_csn(HIGH);
