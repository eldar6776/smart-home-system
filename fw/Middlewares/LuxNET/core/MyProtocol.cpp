...
_Bool protocolSerial2MyMessage(MyMessage &message, char *inputString)
{
	char *str, *p;
	uint8_t index = 0;
	...
	for (str = _strtok_r(inputString, ";", &p); // split using semicolon
	        str && index < 5; // loop while str is not null an max 4 times
	        str = _strtok_r(NULL, ";", &p), index++ // get subsequent tokens
	    ) {
		switch (index) {
...
{
	(void)snprintf(_fmtBuffer, (uint8_t)MY_GATEWAY_MAX_SEND_LENGTH,
	                 "%u;%u;%u;%u;%u;%s\n", message.getSender(),
...
_Bool protocolMQTT2MyMessage(MyMessage &message, char *topic, uint8_t *payload, const unsigned int length)
{
	char *str, *p;
	uint8_t index = 0;
	message.setSender(GATEWAY_ADDRESS);
	message.setLast(GATEWAY_ADDRESS);
	message.setEcho(false);
	for (str = _strtok_r(topic + strlen(MY_MQTT_SUBSCRIBE_TOPIC_PREFIX) + 1, "/", &p);
	        str && index < 5;
	        str = _strtok_r(NULL, "/", &p), index++
	    ) {
		switch (index) {
		case 0:
			// Node id
