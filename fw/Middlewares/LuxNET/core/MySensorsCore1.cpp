...
//	hwInit();
	const _Bool hwInitResult = hwInit();
...
#if defined(F_CPU)
	CORE_DEBUG(PSTR("MCO:BGN:INIT " MY_NODE_TYPE ",CP=" MY_CAPABILITIES ",FQ=%hu,REL=%u,VER="
	                MYSENSORS_LIBRARY_VERSION "\n"), (uint16_t)(F_CPU/1000000UL),
	           MYSENSORS_LIBRARY_VERSION_PRERELEASE_NUMBER);
#else
	CORE_DEBUG(PSTR("MCO:BGN:INIT " MY_NODE_TYPE ",CP=" MY_CAPABILITIES ",FQ=NA,REL=%u,VER="
	                MYSENSORS_LIBRARY_VERSION "\n"), MYSENSORS_LIBRARY_VERSION_PRERELEASE_NUMBER);
#endif
	if (!hwInitResult) {
...
#if ((defined(MY_HW_TX_LED_PORT) && defined(MY_HW_TX_LED_PIN))\
|| 	(defined(MY_HW_RX_LED_PORT)  && defined(MY_HW_RX_LED_PIN))\
|| 	(defined(MY_HW_ERR_LED_PORT) && defined(MY_HW_ERR_LED_PIN)))
	ledsInit();
#endif
	...
	// Note: _coreConfig.isMetric is _Bool, hence empty EEPROM (=0xFF) evaluates to true (default)
...
#if defined(MY_SENSOR_NETWORK)
	CORE_DEBUG(PSTR("MCO:BGN:INIT OK,TSP=%u\n"), isTransportReady() &&
...
_Bool _sendRoute(MyMessage &message)
{
#if defined(MY_CORE_ONLY)
	(void)message;
#endif
...
_Bool send(MyMessage &message, const _Bool requestEcho)
{
...
_Bool present(const uint8_t childSensorId, const mysensors_sensor_t sensorType,
             const char *description,
             const _Bool requestEcho)
{
...
_Bool sendSketchInfo(const char *name, const char *version, const _Bool requestEcho)
{
	_Bool result = true;
	if (name) {
...
_Bool request(const uint8_t childSensorId, const uint8_t variableType, const uint8_t destination)
{
	return _sendRoute(build(_msgTmp, destination, childSensorId, C_REQ, variableType).set(""));
}

_Bool requestTime(const _Bool requestEcho)
{
...
_Bool wait(const uint32_t waitingMS, const mysensors_command_t cmd)
{
#if defined(MY_DEBUG_VERBOSE_CORE)
	if (waitLock) {
		CORE_DEBUG(PSTR("!MCO:WAI:RC=%u\n"), waitLock);	// recursive call detected
...
_Bool wait(const uint32_t waitingMS, const mysensors_command_t cmd, const uint8_t msgType)
{
#if defined(MY_DEBUG_VERBOSE_CORE)
	if (waitLock) {
		CORE_DEBUG(PSTR("!MCO:WAI:RC=%u\n"), waitLock);	// recursive call detected
...
void doYield(void)
{
	...
	//hwWatchdogReset();
	yield();
#if ((defined(MY_HW_TX_LED_PORT) && defined(MY_HW_TX_LED_PIN))||(defined(MY_HW_RX_LED_PORT)&&defined(MY_HW_RX_LED_PIN))||(defined(MY_HW_ERR_LED_PORT)&&defined(MY_HW_ERR_LED_PIN)))
	ledsProcess();
#endif
    if((HAL_GetTick() - cnt) >= 30000)
    {
        cnt = HAL_GetTick();
        presentation();
    }
    
    if((HAL_GetTick() - tmr) >= 2000)
    {
        tmr = HAL_GetTick();
        transportPingNode(BROADCAST_ADDRESS);
    }
}

int8_t _sleep(const uint32_t sleepingMS, const _Bool smartSleep, const uint8_t interrupt1,
              const uint8_t mode1, const uint8_t interrupt2, const uint8_t mode2)
{
	CORE_DEBUG(PSTR("MCO:SLP:MS=%lu,SMS=%u,I1=%u,M1=%u,I2=%u,M2=%u\n"), sleepingMS, smartSleep,
...
int8_t sleep(const uint8_t interrupt1, const uint8_t mode1, const uint8_t interrupt2,
             const uint8_t mode2, const uint32_t sleepingMS, const _Bool smartSleep)
{
...
void _nodeLock(const char *str)
{
	...
	while (1) {
		...
		CORE_DEBUG(PSTR("MCO:NLK:NODE LOCKED. TO UNLOCK, GND PIN %u AND RESET\n"),
		           MY_NODE_UNLOCK_PIN);
		doYield();
...
void _checkNodeLock(void)
{
	...
	if (hwReadConfig(EEPROM_NODE_LOCK_COUNTER_ADDRESS) == 0) {
		...
		GPIO_SetPinMode(MY_NODE_UNLOCK_PIN, INPUT_PULLUP);
		...
		if (hwDigitalRead(MY_NODE_UNLOCK_PIN) == 0) {
			...
			// Disable pullup
			GPIO_SetPinMode(MY_NODE_UNLOCK_PIN, INPUT);
			setIndication(INDICATION_ERR_LOCKED);
