...
#if defined(F_CPU)
	CORE_DEBUG(PSTR("MCO:BGN:INIT ##MY_NODE_TYPE##,CP=##MY_CAPABILITIES##,FQ=%hu,REL=%u,VER=##MYSENSORS_LIBRARY_VERSION##\n"), 
    (uint16_t)(F_CPU/1000000UL),MYSENSORS_LIBRARY_VERSION_PRERELEASE_NUMBER);
#else
	CORE_DEBUG(PSTR("MCO:BGN:INIT ##MY_NODE_TYPE##,CP=##MY_CAPABILITIES##,FQ=NA,REL=%u,VER=##MYSENSORS_LIBRARY_VERSION##\n"), 
...
void presentNode(void)
{
	...
	(void)present(NODE_SENSOR_ID, S_LUX_REPEATER_NODE);
#else
	(void)present(NODE_SENSOR_ID, S_LUX_NODE);
#endif
#else
...
{
	CORE_DEBUG(PSTR("MCO:SLP:MS=%ud,SMS=%u,I1=%u,M1=%u,I2=%u,M2=%u\n"), sleepingMS, smartSleep,
	...
	CORE_DEBUG(PSTR("MCO:SLP:WUP=%hhu\n"), result);	// sleep wake-up
#if defined(MY_SENSOR_NETWORK)
