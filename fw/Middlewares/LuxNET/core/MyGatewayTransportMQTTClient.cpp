...
static _Bool _MQTT_connecting = true;
static _Bool _MQTT_available = false;
...
_Bool gatewayTransportSend(MyMessage &message)
{
	...
	const _Bool retain = message.getCommand() == C_SET ||
	                    (message.getCommand() == C_INTERNAL && message.getType() == I_BATTERY_LEVEL);
#else
	const _Bool retain = false;
...
_Bool gatewayTransportConnect(void)
{
#if defined(MY_GATEWAY_ESP8266) || defined(MY_GATEWAY_ESP32)
