...
#define _autoFindParent (_Bool)(MY_PARENT_NODE_ID == AUTO)				//!<  returns true if static parent id is undefined
#define isValidDistance(_distance) (_Bool)(_distance!=DISTANCE_INVALID)	//!<  returns true if distance is valid
#define isValidParent(_parent) (_Bool)(_parent != AUTO)					//!<  returns true if parent is valid
...
typedef struct {
	...
	// 8 bits
	_Bool findingParentNode : 1;							//!< flag finding parent node is active
	_Bool preferredParentFound : 1;					//!< flag preferred parent found
	_Bool uplinkOk : 1;											//!< flag uplink ok
	_Bool pingActive : 1;										//!< flag ping active
	_Bool transportActive : 1;								//!< flag transport active
	...
	_Bool msgReceived : 1;										//!< flag message received
...
*/
_Bool transportCheckUplink(const _Bool force = false);
...
*/
_Bool transportWaitUntilReady(const uint32_t waitingMS = 0);
/**
