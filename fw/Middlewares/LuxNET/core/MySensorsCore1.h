...
typedef struct {
	...
	// 8 bit
	_Bool nodeRegistered : 1;				//!< Flag node registered
	_Bool presentationSent : 1;				//!< Flag presentation sent
 ...
 */
_Bool present(const uint8_t sensorId, const mysensors_sensor_t sensorType,
             const char *description = "",
             const _Bool requestEcho = false);
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool sendSketchInfo(const char *name, const char *version, const _Bool requestEcho = false);
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool send(MyMessage &msg, const _Bool requestEcho = false);
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool sendBatteryLevel(const uint8_t level, const _Bool requestEcho = false);
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool sendHeartbeat(const _Bool requestEcho = false);
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool sendSignalStrength(const int16_t level, const _Bool requestEcho = false);
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool sendTXPowerLevel(const uint8_t level, const _Bool requestEcho = false);
...
* @return true Returns true if message reached the first stop on its way to destination.
*/
_Bool request(const uint8_t childSensorId, const uint8_t variableType,
 ...
 * @return true Returns true if message reached the first stop on its way to destination.
 */
_Bool requestTime(const _Bool requestEcho = false);
 ...
 */
int8_t sleep(const uint32_t sleepingMS, const _Bool smartSleep = false);
...
int8_t sleep(const uint8_t interrupt1, const uint8_t mode1, const uint8_t interrupt2,
             const uint8_t mode2, const uint32_t sleepingMS = 0, const _Bool smartSleep = false);
...
* @return Interrupt number if wake up was triggered by pin change, @ref MY_WAKE_UP_BY_TIMER if wake up was triggered by timer, @ref MY_SLEEP_NOT_POSSIBLE if sleep was not possible (e.g. ongoing FW update)
*/
int8_t _sleep(const uint32_t sleepingMS, const _Bool smartSleep = false,
...
static inline MyMessage& build(MyMessage &msg, const uint8_t destination, const uint8_t sensor,
                               const mysensors_command_t command, const uint8_t type, const _Bool requestEcho = false)
{
