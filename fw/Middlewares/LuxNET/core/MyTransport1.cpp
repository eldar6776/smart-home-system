...
#else
		if (MY_NODE_ID != AUTO) {
			TRANSPORT_DEBUG(PSTR("TSM:INIT:STATID=%u\n"),(uint8_t)MY_NODE_ID);
			// Set static ID
...
void stUplinkUpdate(void)
{
	...
	if (_transportSM.pingResponse != INVALID_HOPS) {
		...
		if (_transportSM.pingResponse != _transportConfig.distanceGW) {
			TRANSPORT_DEBUG(PSTR("TSM:UPL:DGWC,O=%u,N=%u\n"), _transportConfig.distanceGW,
...
void stReadyTransition(void)
{
	...
	TRANSPORT_DEBUG(PSTR("TSM:READY:ID=%u,PAR=%u,DIS=%u\n"),
...
void stFailureTransition(void)
{
	...
	TRANSPORT_DEBUG(PSTR("TSM:FAIL:CNT=%u\n"),_transportSM.failureCounter);
...
_Bool isTransportReady(void)
{
	return _transportSM.uplinkOk;
}

_Bool isTransportExtendedFailure(void)
{
	return _transportSM.failureCounter == MY_TRANSPORT_MAX_TSM_FAILURES;
}

_Bool isTransportSearchingParent(void)
{
...
_Bool transportWaitUntilReady(const uint32_t waitingMS)
{
...
_Bool transportCheckUplink(const _Bool force)
{
	...
	if (hopsCount != INVALID_HOPS) {
		...
		if (hopsCount != _transportConfig.distanceGW) {
			TRANSPORT_DEBUG(PSTR("TSF:CKU:DGWC,O=%u,N=%u\n"), _transportConfig.distanceGW,
...
_Bool transportAssignNodeID(const uint8_t newNodeId)
{
	// verify if ID valid
	if (newNodeId != GATEWAY_ADDRESS && newNodeId != AUTO) {
		...
		TRANSPORT_DEBUG(PSTR("TSF:SID:OK,ID=%u\n"),newNodeId);	// Node ID assigned
		return true;
	} else {
		TRANSPORT_DEBUG(PSTR("!TSF:SID:FAIL,ID=%u\n"),newNodeId);	// ID is invalid, cannot assign ID
		setIndication(INDICATION_ERR_NET_FULL);
		return false;
	}
}

_Bool transportRouteMessage(MyMessage &message)
{
	...
	if (destination == GATEWAY_ADDRESS) {
		...
		if (route == AUTO) {
//			TRANSPORT_DEBUG(PSTR("!TSF:RTE:%u UNKNOWN\n"), destination);	// route unknown
#if !defined(MY_GATEWAY_FEATURE)
	...
	// send message
	const _Bool result = transportSendWrite(route, message);
#if !defined(MY_GATEWAY_FEATURE)
	// update counter
...
_Bool transportSendRoute(MyMessage &message)
{
	_Bool result = false;
	if (isTransportReady()) {
...
_Bool transportWait(const uint32_t waitingMS, const uint8_t cmd, const uint8_t msgType)
{
...
uint8_t transportPingNode(const uint8_t targetId)
{
	if(!_transportSM.pingActive) {
		TRANSPORT_DEBUG(PSTR("TSF:PNG:SEND,TO=%u\n"), targetId);
...
void transportProcessMessage(void)
{
	...
	TRANSPORT_DEBUG(PSTR("TSF:MSG:READ,%u-%u-%u,s=%u,c=%u,t=%u,pt=%u,l=%u,sg=%u:%s\n"),
	...
	if (destination == _transportConfig.nodeId) {
		...
		if(!_msg.isEcho()) {
			...
			if (command == C_INTERNAL) {
					...
					if (_transportSM.findingParentNode) {	// only process if find parent active
						...
						if (isValidDistance(distance)) {
							...
							if (((isValidDistance(distance) && distance < _transportConfig.distanceGW) || (!_autoFindParent &&
								...
								TRANSPORT_DEBUG(PSTR("TSF:MSG:FPAR OK,ID=%u,D=%u\n"), _transportConfig.parentNodeId,
								                _transportConfig.distanceGW);
							}
						}
					} else {
				...
				// general
				if (type == I_PING) {
//					TRANSPORT_DEBUG(PSTR("TSF:MSG:PINGED,ID=%u,HP=%u\n"), sender,
//					                _msg.getByte()); // node pinged
				...
				if (type == I_PONG) {
					if (_transportSM.pingActive) {
						...
						TRANSPORT_DEBUG(PSTR("TSF:MSG:PONG RECV,HP=%u\n"),
			...
			if (isTransportReady()) {
				...
				if (type == I_FIND_PARENT_REQUEST) {
					...
					if (sender != _transportConfig.parentNodeId) {	// no circular reference
						TRANSPORT_DEBUG(PSTR("TSF:MSG:FPAR REQ,ID=%u\n"), sender);	// FPAR: find parent request
		...
		if (isTransportReady()) {
			if (command == C_INTERNAL) {
				if (type == I_PING || type == I_PONG) {
					...
					TRANSPORT_DEBUG(PSTR("TSF:MSG:REL PxNG,HP=%u\n"), hopsCnt);
					if (hopsCnt != MAX_HOPS) {
						hopsCnt++;
						_msg.set(hopsCnt);
					}
...
_Bool transportSendWrite(const uint8_t to, MyMessage &message)
{
	...
	const uint8_t totalMsgLength = HEADER_SIZE + ( message.getSigned() ? MAX_PAYLOAD_SIZE :
	                               message.getLength() );
	const _Bool noACK = _transportConfig.passiveMode || (to == BROADCAST_ADDRESS);
	// send
	setIndication(INDICATION_TX);
	const _Bool result = transportHALSend(to, &message, totalMsgLength,
	                                     noACK);

	TRANSPORT_DEBUG(PSTR("%sTSF:MSG:SEND,%u-%u-%u-%u,s=%u,c=%u,t=%u,pt=%u,l=%u,sg=%u,ft=%u,st=%s:%s\n"),
...
void transportReportRoutingTable(void)
{
	...
	for (uint16_t cnt = 0; cnt < SIZE_ROUTES; cnt++) {
		...
		if (route != BROADCAST_ADDRESS) {
			TRANSPORT_DEBUG(PSTR("TSF:RRT:ROUTE N=%u,R=%u\n"), cnt, route);
...
void transportTogglePassiveMode(const _Bool OnOff)
{
#if !defined (MY_PASSIVE_NODE)
...
int16_t transportSignalReport(const char command)
{
	...
	TRANSPORT_DEBUG(PSTR("TSF:SIR:CMD=%u,VAL=%hu\n"), reportCommand, result);
	return result;
#else
	(void)command;
	return 0;
