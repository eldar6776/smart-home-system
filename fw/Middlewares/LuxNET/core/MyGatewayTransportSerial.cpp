...
bool gatewayTransportSend(MyMessage &message)
{
    ...
    char * pmsg = protocolMyMessage2Serial(message);
	uint16_t len = strlen(pmsg);
    HAL_UART_Transmit(&huart1, (std::uint8_t *)pmsg, len, 100);
...
bool gatewayTransportAvailable(void)
{
	while ((__HAL_UART_GET_FLAG(&huart1, UART_FLAG_RXNE) ? SET : RESET) == SET) {
		// get the new byte:
        char inChar = (char) (huart1.Instance->DR & (uint8_t)0x00FF);
