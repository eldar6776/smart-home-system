...
extern _Bool signerAtsha204SoftInit(void);
extern _Bool signerAtsha204SoftCheckTimer(void);
extern _Bool signerAtsha204SoftGetNonce(MyMessage &msg);
extern void signerAtsha204SoftPutNonce(MyMessage &msg);
extern _Bool signerAtsha204SoftVerifyMsg(MyMessage &msg);
extern _Bool signerAtsha204SoftSignMsg(MyMessage &msg);
...
extern _Bool signerAtsha204Init(void);
extern _Bool signerAtsha204CheckTimer(void);
extern _Bool signerAtsha204GetNonce(MyMessage &msg);
extern void signerAtsha204PutNonce(MyMessage &msg);
extern _Bool signerAtsha204VerifyMsg(MyMessage &msg);
extern _Bool signerAtsha204SignMsg(MyMessage &msg);
...
#endif
static _Bool skipSign(MyMessage &msg);
...
static _Bool signerInternalProcessPresentation(MyMessage &msg);
static _Bool signerInternalProcessNonceRequest(MyMessage &msg);
static _Bool signerInternalProcessNonceResponse(MyMessage &msg);
#if (defined(MY_ENCRYPTION_FEATURE) || defined(MY_SIGNING_FEATURE)) &&\
    !defined(MY_SIGNING_SIMPLE_PASSWD)
static _Bool signerInternalValidatePersonalization(void);
#endif
...
_Bool signerProcessInternal(MyMessage &msg)
{
	_Bool ret;
	switch (msg.getType()) {
...
_Bool signerCheckTimer(void)
{
	return signerBackendCheckTimer();
}

// cppcheck-suppress constParameter
_Bool signerSignMsg(MyMessage &msg)
{
#if defined(MY_SIGNING_FEATURE)
	_Bool ret;
	...
	if (DO_SIGN(msg.getDestination()) && msg.getSender() == getNodeId()) {
		...
		SIGN_DEBUG(PSTR("SGN:SGN:NREQ=%u\n"),
		           msg.getDestination()); // Do not sign message as it is not req
		ret = true;
	} else {
		SIGN_DEBUG(PSTR("SGN:SGN:%u!=%u NUS\n"), msg.getSender(),
...
// cppcheck-suppress constParameter
_Bool signerVerifyMsg(MyMessage &msg)
{
	_Bool verificationResult = true;
...
static _Bool skipSign(MyMessage &msg)
{
	...
	if (ret) {
		SIGN_DEBUG(PSTR("SGN:SKP:%s CMD=%u,TYPE=%u\n"), msg.isEcho() ? "ECHO" : "MSG",
		           msg.getCommand(),
...
static _Bool signerInternalProcessPresentation(MyMessage &msg)
{
	...
	if (msg.data[1] & SIGNING_PRESENTATION_REQUIRE_SIGNATURES) {
		SIGN_DEBUG(PSTR("SGN:PRE:SGN REQ,TO=%u\n"),
		           sender); // Inform node that we require signatures
	} else {
		SIGN_DEBUG(PSTR("SGN:PRE:SGN NREQ,TO=%u\n"),
		           sender); // Inform node that we do not require signatures
	}
	if (msg.data[1] & SIGNING_PRESENTATION_REQUIRE_WHITELISTING) {
		SIGN_DEBUG(PSTR("SGN:PRE:WHI REQ,TO=%u\n"),
		           sender); // Inform node that we require whitelisting
	} else {
		SIGN_DEBUG(PSTR("SGN:PRE:WHI NREQ,TO=%u\n"),
	...
	prepareSigningPresentation(msg, sender);
	SIGN_DEBUG(PSTR("SGN:PRE:NSUP,TO=%u\n"),
...
static _Bool signerInternalProcessNonceRequest(MyMessage &msg)
{
...
#if defined(MY_NODE_LOCK_FEATURE)
	nof_nonce_requests++;
	SIGN_DEBUG(PSTR("SGN:NCE:LEFT=%u\n"),
	...
	if (signerBackendGetNonce(msg)) {
		if (!_sendRoute(build(msg, msg.getSender(), NODE_SENSOR_ID, C_INTERNAL, I_NONCE_RESPONSE))) {
			SIGN_DEBUG(PSTR("!SGN:NCE:XMT,TO=%u FAIL\n"), msg.getSender()); // Failed to transmit nonce!
		} else {
			SIGN_DEBUG(PSTR("SGN:NCE:XMT,TO=%u\n"), msg.getSender());
		}
	} else {
...
static _Bool signerInternalProcessNonceResponse(MyMessage &msg)
{
	...
	SIGN_DEBUG(PSTR("SGN:NCE:FROM=%u\n"), msg.getSender());
	if (msg.getSender() != _msgSign.getDestination()) {
		SIGN_DEBUG(PSTR("SGN:NCE:%u!=%u (DROPPED)\n"), _msgSign.getDestination(),
		           msg.getSender());
	} else {
