...
public:
    ...
    explicit I2CEeprom(void);
    bool init(void); //!< setup
	uint8_t readByte(uint16_t addr); //!< read 1 byte from flash memory
	void readBytes(uint16_t addr, void * buf, uint16_t len); //!< read multiple bytes
	void writeByte(uint16_t addr, uint8_t byt); //!< Write 1 byte to flash memory
	void writeBytes(uint16_t addr, const void * buf, uint16_t len); //!< write multiple bytes to flash memory (up to 64K), if define SPIFLASH_SST25TYPE is set AAI Word Programming will be used
	bool busy(void); //!< check if the chip is busy erasing/writing
