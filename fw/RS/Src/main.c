...
#define RFBSZ                               255     // radio receive buffer size
#define RFBCN                               3       // count of receive buffer
/* Private Variable ----------------------------------------------------------*/
uint8_t sysfl   = 0, initfl = 0;
uint32_t rstsrc = 0;
static volatile float adc_refcor = 1.0f;
ask_t	rf;
uint8_t	RemoteData[5];
uint8_t RemoteDataLen;
uint16_t rfcode[RFBCN][RFBSZ];
uint32_t rfstart, rfbcnt, rftmr, rfcnt;
/* Private Macro -------------------------------------------------------------*/
...
float ROOM_GetTemperature(uint16_t adc_value);
/* Program Code  -------------------------------------------------------------*/
/**
  * @brief
  * @param
...
int main(void){
	...
	while(1){
		...
		DISP_Service();
        RS485_Service();
        if (rfstart && (HAL_GetTick()-rftmr >= 7)){
            rfcode[rfcnt][rfbcnt]   = 'E';
            rfcode[rfcnt][rfbcnt+1] =  0;
            if      (rfbcnt == 82){
                EE_WriteBuffer((uint8_t*)&rfcode[rfcnt][0], EE_RFCODE1, (rfbcnt+1)*2);
            }else if(rfbcnt == 156){
                EE_WriteBuffer((uint8_t*)&rfcode[rfcnt][0], EE_RFCODE2, (rfbcnt+1)*2);
            }
            if (++rfcnt >= RFBCN) rfcnt = 0;
            rfstart = 0;
            rfbcnt = 0;
        }
#ifdef	USE_WATCHDOG
...
/**
  * @brief  send to usart system state parsed string
  * @param
  * @retval
  */
...
void HAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
	if(GPIO_Pin == GPIO_PIN_14){
        if (rfbcnt > RFBSZ-2) return;
        if (rfstart == 0) rfstart = 1, __HAL_TIM_SET_COUNTER(&htim2, 0);
        if (HAL_GPIO_ReadPin(GPIOG, GPIO_PIN_14) == GPIO_PIN_RESET){
//            rfcode[rfcnt][rfbcnt++] = 'L';
            rfcode[rfcnt][rfbcnt] = htim2.Instance->CNT;
        } else {
//            rfcode[rfcnt][rfbcnt++] = 'H';
            rfcode[rfcnt][rfbcnt] = htim2.Instance->CNT;
        }
...
static void RAM_Init(void){
    ...
    if (ebuf[EE_INIT_ADDR] != 0){
        ...
        ebuf[EE_LIGHT_CTRL1]   = 1;
        ebuf[EE_LIGHT_CTRL1+1] = 2;
        ebuf[EE_LIGHT_CTRL1+2] = 3;
        ebuf[EE_LIGHT_CTRL1+3] = 4;
        ebuf[EE_LIGHT_CTRL1+4] = 5;
        ebuf[EE_LIGHT_CTRL1+5] = 1;
        ebuf[EE_LIGHT_CTRL1+6] = 2;
        ebuf[EE_LIGHT_CTRL1+7] = 3;

        ebuf[EE_THST_CTRL1]   = 1;
        ebuf[EE_THST_CTRL1+1] = 0;
        ebuf[EE_THST_CTRL1+2] = 0;
        ebuf[EE_THST_CTRL1+3] = 0;
        ebuf[EE_THST_CTRL1+4] = 0;
        ebuf[EE_THST_CTRL1+5] = 0x27;
        ebuf[EE_THST_CTRL1+6] = 0x10;
        ebuf[EE_THST_CTRL1+7] = 0x0F;
        ebuf[EE_THST_CTRL1+8] = 0x89;
        ebuf[EE_THST_CTRL1+9] = 24;
        ebuf[EE_THST_CTRL1+10] = 5;
        ebuf[EE_THST_CTRL1+11] = 32;
        ebuf[EE_THST_CTRL1+12] = 16;
        ebuf[EE_THST_CTRL1+13] = 1;
        ebuf[EE_THST_CTRL1+14] = 0;
        ebuf[EE_THST_CTRL1+15] = 5;
        ebuf[EE_THST_CTRL1+16] = 10;
        ebuf[EE_THST_CTRL1+17] = 15;
        ebuf[EE_THST_CTRL1+18] = 22;
        ebuf[EE_THST_CTRL1+19] = 22;
        ebuf[EE_THST_CTRL1+20] = 1;
    ...
    EE_ReadBuffer((uint8_t*)&rfcode[0][0], EE_RFCODE1, 0x200);
    EE_ReadBuffer((uint8_t*)&rfcode[1][0], EE_RFCODE2, 0x200);
    EE_ReadBuffer((uint8_t*)&rfcode[2][0], EE_RFCODE3, 0x200);
    ...
    if ((rf1 == 82)||(rf1 == 156)){
        for (x=0; x<rf1; x++){
            if (x & 0x1) HAL_GPIO_WritePin(GPIOD, GPIO_PIN_2, GPIO_PIN_RESET);
            else         HAL_GPIO_WritePin(GPIOD, GPIO_PIN_2, GPIO_PIN_SET);
            while(cnt < htim2.Instance->CNT){
            }
            cnt = htim2.Instance->CNT;
            htim2.Instance->CNT = rfcode[0][x++];
        }
    } else {
        memset(&rfcode[0][0],0,0x100);
    ...
    if ((rf2 == 82)||(rf2 == 156)){
        for (x=0; x<rf2; x++){
            ...
            cnt = htim2.Instance->CNT;
            htim2.Instance->CNT = rfcode[1][x++];
        }
    } else {
        memset(&rfcode[1][0],0,0x100);
        HAL_GPIO_WritePin(GPIOD, GPIO_PIN_2, GPIO_PIN_RESET);
    }
    ...
    if ((rf3 == 82)||(rf3 == 156)){
        for (x=0; x<rf3; x++){
            ...
            cnt = htim2.Instance->CNT;
            htim2.Instance->CNT = rfcode[2][x++];
        }
    } else {
        memset(&rfcode[2][0],0,0x100);
        HAL_GPIO_WritePin(GPIOD, GPIO_PIN_2, GPIO_PIN_RESET);
    }
}
/**
...
//                ntc_temp = adc_calc*10;
//                if (thst.mv_temp != ntc_temp + thst.mv_offset){
//                    thst.mv_temp = ntc_temp + thst.mv_offset;
//                    MVUpdateSet();
//                }
            }
        }
        else if (adc_cnt == 1U) tmp = HAL_ADC_GetValue(&hadc3); // DUMMU READ
        else if (adc_cnt == 2U) light_ldr = 255 - (HAL_ADC_GetValue(&hadc3) / 16);
        if (++adc_cnt > 2U) adc_cnt = 0U;
    }
}
/**
  * @brief
