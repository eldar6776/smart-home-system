...
#define OS_SIZEOF_INT             (4)   // Size of integer in bytes
  ...
  #else
    #error "Your GCC does not support __ARMEB__ or __ARMEL__."
  #endif
#endif
...
#define OS_EnterRegion            OS_EnterRegionFunc         // A function is used instead of the macro
...
/*****  End of configuration settings *******************************/
...
#if (defined __ARM_ARCH_6M__)  // Cortex-M0
  #define OS_DI() { __asm volatile ("CPSID i \n\t"  \
                                    :               \
                                    :               \
                                    :               \
                                    );              \
                  }
  #define OS_EI() { __asm volatile ("CPSIE i \n\t"  \
                                    :               \
                                    :               \
                                    :               \
                                    );              \
  ...
  //
  #define OS_DI() { __asm volatile ("movs r0, #128    \n\t"  \
                                    "msr  basepri, r0 \n\t"  \
                                    "nop              \n\t"  \
                                    "nop              \n\t"  \
                                    :                        \
  ...
  #define OS_EI() { __asm volatile ("movs r0, #0      \n\t"  \
                                    "msr  basepri, r0 \n\t"  \
                                    "nop              \n\t"  \
                                    "nop              \n\t"  \
                                    :                        \
...
#elif (defined __ARM_ARCH_7EM__) // Cortex-M4/M4F/M7
  //
  ...
  // disable interrupts while accessing BASEPRI. Effectively, zero-latency ISRs
  // will be enabled after each call to OS_DI() / OS_EI(). As other CPUs and revisions
  // are not affected, CPSID and CPSIE instructions may be removed if using any other
  // CPU than Cortex-M7 r0p1.
  // The NOPs should flush the pipeline to ensure that interrupts
  // are disabled before the next instruction is executed.
  //
  #define OS_DI() { __asm volatile ("movs r0, #128    \n\t"  \
...
#else
  #error "Please check that __ARM_ARCH_6M__ or __ARM_ARCH_7M__ or __ARM_ARCH_7EM__ is defined!"
#endif
...
*       Implemented here
*/
#define OS_INTERRUPT_MaskGlobal()      __asm("cpsid I")
#define OS_INTERRUPT_UnmaskGlobal()    __asm("cpsie I")

#define OS_INTERRUPT_PreserveGlobal(p) (*(p) = (OS_U32) OS_GetPRIMASK())

#define OS_INTERRUPT_RestoreGlobal(p)  OS_SetPRIMASK(*(p))

#define OS_INTERRUPT_PreserveAndMaskGlobal(p)  { \
  (*(p) = (OS_U32) OS_GetPRIMASK());             \
  __asm("cpsid I");                              \
}
...
*/
#if (defined __ARM_ARCH_6M__)     // Cortex-M0
  #define OS_INT_PRIO_PRESERVE(p)   (*(p) = (OS_U32) OS_GetPRIMASK())
  #define OS_INT_PRIO_RESTORE(p)    OS_SetPRIMASK(*(p))
#elif (defined __ARM_ARCH_7M__)   // Cortex-M3
  #define OS_INT_PRIO_PRESERVE(p)   { __asm volatile ("mrs  R1, basepri  \n\t" \
                                                      "str  R1, [%0]     \n\t" \
                                                      :                        \
                                                      : "r" (p)                \
                                                      : "r1","memory"          \
  ...
  #define OS_INT_PRIO_RESTORE(p)    { __asm volatile ("ldr  R1, [%0]\n\t"      \
                                                      "msr  basepri, R1\n\t"   \
                                                      :                        \
                                                      : "r" (p)                \
                                                      : "r1","memory"          \
...
#define OS_VERSION_GENERIC (42400u)       /*  Generic version 4.24 */
...
#define OS_TRACE_API_OFFSET              (32u)  // API IDs start at this offset. This has been done to maintain compatibility of API-Ids with embOSView, which uses the reserved area of SYSVIEW (0..31)
...
typedef struct MEM_CONFIG_STRUCT {
  OS_U32 ROM_BaseAddr;
  OS_U32 ROM_Size;
  OS_U32 RAM_BaseAddr;
  OS_U32 RAM_Size;
...
typedef struct MPU_API_LIST_STRUCT {
  void   (*pfEnable)              (void);
...
void                OS_MPU_AddRegion          (OS_TASK* pTask, OS_U32 BaseAddr, OS_U32 Size, OS_U32 Permissions)                                                           OS_SECTION_ATTRIBUTE;
...
void                OS_MPU_CallDeviceDriver   (OS_U32 Index, void* Param)                                                                                                  OS_SECTION_ATTRIBUTE;
OS_MPU_THREAD_STATE OS_MPU_GetThreadState     (void)                                                                                                                       OS_SECTION_ATTRIBUTE;
...
*/
#ifdef OSGLOBAL_C
  #define OS_EXTERN        /* Define variables if included by OSGLOBAL.C */
#else
...
#define OS_TimeDex           OS_Global.TimeDex
  ...
  #if (OS_SUPPORT_POWER != 0)
    OS_EXTERN OS_SADDR OS_U8 OS_POWER_Levels[OS_POWER_NUM_COUNTERS];  /*lint !e9075 MISRA C:2012 Rule 8.4, required  */
  #endif
...
**********************************************************************
*/
void     OS_AddOnTerminateHook       (OS_ON_TERMINATE_HOOK * pHook, OS_ON_TERMINATE_FUNC * pfUser) OS_SECTION_ATTRIBUTE;
void     OS_Delay                    (OS_TIME ms)                                                  OS_SECTION_ATTRIBUTE;  /* OSKern.c              */
void     OS_DelayUntil               (OS_TIME t)                                                   OS_SECTION_ATTRIBUTE;  /* OSKern.c              */
void     OS_Delayus                  (OS_U16  us)                                                  OS_SECTION_ATTRIBUTE;  /* OS_Delayus.c          */
OS_U8    OS_GetSuspendCnt            (OS_CONST_PTR OS_TASK* pTask)                                 OS_SECTION_ATTRIBUTE;  /* OS_GetSuspendCnt.c    */
OS_BOOL  OS_IsTask                   (OS_CONST_PTR OS_TASK* pTask)                                 OS_SECTION_ATTRIBUTE;  /* OSIsTask.c            */
void     OS_Resume                   (OS_TASK* pTask)                                              OS_SECTION_ATTRIBUTE;  /* OS_Suspend.c          */
void     OS_ResumeAllSuspendedTasks  (void)                                                        OS_SECTION_ATTRIBUTE;  /* OS_ResumeAllTasks.c   */
void     OS_SetTaskName              (OS_TASK * pTask, OS_ROM_DATA const char* s)                  OS_SECTION_ATTRIBUTE;
void     OS_SetPriority              (OS_TASK* pTask, OS_PRIO Priority)                            OS_SECTION_ATTRIBUTE;  /* OSSetP.c   */
void     OS_SetInitialSuspendCnt     (OS_U8 SuspendCnt)                                            OS_SECTION_ATTRIBUTE;  /* OS_SetSuspendCnt.c    */
void     OS_Suspend                  (OS_TASK* pTask)                                              OS_SECTION_ATTRIBUTE;  /* OS_Suspend.c          */
void     OS_SuspendAllTasks          (void)                                                        OS_SECTION_ATTRIBUTE;  /* OS_SuspendAllTasks.c  */
void     OS_TerminateTask            (OS_TASK* pTask)                                              OS_SECTION_ATTRIBUTE;  /* OSTerm.c              */
void     OS_WakeTask                 (OS_TASK* pTask)                                              OS_SECTION_ATTRIBUTE;  /* OSWake.c              */
void     OS_Yield                    (void)                                                        OS_SECTION_ATTRIBUTE;  /* OS_Yield              */
...
OS_ROM_DATA const char* OS_GetCPU    (void) OS_SECTION_ATTRIBUTE;
OS_ROM_DATA const char* OS_GetLibMode(void) OS_SECTION_ATTRIBUTE;
OS_ROM_DATA const char* OS_GetModel  (void) OS_SECTION_ATTRIBUTE;
OS_ROM_DATA const char* OS_GetLibName(void) OS_SECTION_ATTRIBUTE;
...
OS_BOOL OS_InInterrupt(void);
...
#if (OS_TRACKNAME != 0)
  OS_ROM_DATA const char* OS_GetTaskName(OS_CONST_PTR OS_TASK OS_TCBPTR *pTask) OS_SECTION_ATTRIBUTE;
#else
  #define OS_GetTaskName(pt)  ("n/a")
#endif

#if (OS_CHECKSTACK != 0)
  unsigned int      OS_GetStackSpace(OS_CONST_PTR OS_TASK OS_TCBPTR *pTask) OS_SECTION_ATTRIBUTE;
  unsigned int      OS_GetStackUsed (OS_CONST_PTR OS_TASK OS_TCBPTR *pTask) OS_SECTION_ATTRIBUTE;
  unsigned int      OS_GetStackSize (OS_CONST_PTR OS_TASK OS_TCBPTR *pTask) OS_SECTION_ATTRIBUTE;
  void OS_STACKPTR* OS_GetStackBase (OS_CONST_PTR OS_TASK OS_TCBPTR *pTask) OS_SECTION_ATTRIBUTE;
#else
  ...
  void OS_STACKPTR* OS_GetSysStackBase (void) OS_SECTION_ATTRIBUTE;  /* OS_StackInfo.c */
  unsigned int      OS_GetSysStackSize (void) OS_SECTION_ATTRIBUTE;  /* OS_StackInfo.c */
#else
  ...
  unsigned int OS_GetSysStackSpace(void) OS_SECTION_ATTRIBUTE;  /* OS_SysStackInfo.c */
  unsigned int OS_GetSysStackUsed (void) OS_SECTION_ATTRIBUTE;  /* OS_SysStackInfo.c */
#else
  ...
  void OS_STACKPTR* OS_GetIntStackBase(void) OS_SECTION_ATTRIBUTE;  /* OS_StackInfo.c */
  unsigned int      OS_GetIntStackSize(void) OS_SECTION_ATTRIBUTE;  /* OS_StackInfo.c */
#else
  ...
  unsigned int OS_GetIntStackSpace(void) OS_SECTION_ATTRIBUTE; /* OS_IntStackInfo.c */
  unsigned int OS_GetIntStackUsed (void) OS_SECTION_ATTRIBUTE; /* OS_IntStackInfo.c */
#else
  ...
  OS_U8 OS_GetTimeSliceRem(OS_CONST_PTR OS_TASK OS_TCBPTR * pTask) OS_SECTION_ATTRIBUTE;
...
OS_PRIO OS_GetPriority(OS_CONST_PTR OS_TASK * pTask) OS_SECTION_ATTRIBUTE;  /* OS_GetPrio.c */
#define OS_GET_PRIORITY(pt) (pt)->Priority

int     OS_GetNumTasks(void) OS_SECTION_ATTRIBUTE;  /* OS_GetNumTasks.c   */

OS_TASK* OS_TaskIndex2Ptr(int TaskIndex) OS_SECTION_ATTRIBUTE;  /* OS_TaskIndex2Ptr.c */

#if (OS_PROFILE >= 1)
  OS_U32 OS_STAT_GetExecTime_Cycles (const OS_TASK *pTask) OS_SECTION_ATTRIBUTE; /* OSStat.c   */
  int    OS_STAT_GetLoad            (const OS_TASK *pTask) OS_SECTION_ATTRIBUTE; /* OSStatGL.c */
  void   OS_STAT_Sample             (void)                 OS_SECTION_ATTRIBUTE; /* OSStat.c   */
...
#if (OS_SUPPORT_STAT != 0)
  OS_U32 OS_STAT_GetNumActivations  (const OS_TASK *pTask) OS_SECTION_ATTRIBUTE;  /* OSStat.c   */
  OS_U32 OS_STAT_GetNumPreemptions  (const OS_TASK *pTask) OS_SECTION_ATTRIBUTE;  /* OSStat.c   */
#else
...
int OS_WAIT_OBJ_GetSize           (void) OS_SECTION_ATTRIBUTE;
int OS_WAIT_OBJ_EX_GetSize        (void) OS_SECTION_ATTRIBUTE;
int OS_WAIT_LIST_GetSize          (void) OS_SECTION_ATTRIBUTE;
int OS_EXTEND_TASK_CONTEXT_GetSize(void) OS_SECTION_ATTRIBUTE;
int OS_TASK_GetSize               (void) OS_SECTION_ATTRIBUTE;
int OS_REGS_GetSize               (void) OS_SECTION_ATTRIBUTE;
int OS_TICK_HOOK_GetSize          (void) OS_SECTION_ATTRIBUTE;
int OS_RSEMA_GetSize              (void) OS_SECTION_ATTRIBUTE;
int OS_CSEMA_GetSize              (void) OS_SECTION_ATTRIBUTE;
int OS_MAILBOX_GetSize            (void) OS_SECTION_ATTRIBUTE;
int OS_Q_GetSize                  (void) OS_SECTION_ATTRIBUTE;
int OS_MEMF_GetSize               (void) OS_SECTION_ATTRIBUTE;
int OS_EVENT_GetSize              (void) OS_SECTION_ATTRIBUTE;
int OS_TRACE_ENTRY_GetSize        (void) OS_SECTION_ATTRIBUTE;

#if (OS_SUPPORT_TIMER != 0)
  int OS_TIMER_GetSize            (void) OS_SECTION_ATTRIBUTE;
  int OS_TIMER_EX_GetSize         (void) OS_SECTION_ATTRIBUTE;
#endif
...
void OS_EnterRegionFunc(void) OS_SECTION_ATTRIBUTE;  /* in OS_EnterRegionFunc.c */
...
void OS_RestoreI(void) OS_SECTION_ATTRIBUTE;  /* OS_Kern.c */
...
#if (OS_DEBUG != 0)
  void OS_AssertCPUInISRMode(void) OS_SECTION_ATTRIBUTE;  /* OSKern.c   */
#endif
...
int      OS_Use             (OS_RSEMA * pRSema)                  OS_SECTION_ATTRIBUTE;  /* OSRsem.c      */
int      OS_UseTimed        (OS_RSEMA * pRSema, OS_TIME TimeOut) OS_SECTION_ATTRIBUTE;  /* OS_UseTimed.c */
void     OS_Unuse           (OS_RSEMA * pRSema)                  OS_SECTION_ATTRIBUTE;  /* OSRsem.c      */
char     OS_Request         (OS_RSEMA * pRSema)                  OS_SECTION_ATTRIBUTE;  /* OSRsemRQ.c    */
int      OS_GetSemaValue    (OS_CONST_PTR OS_RSEMA * pRSema)     OS_SECTION_ATTRIBUTE;  /* OSRSemGV.c    */
OS_TASK* OS_GetResourceOwner(OS_CONST_PTR OS_RSEMA * pRSema)     OS_SECTION_ATTRIBUTE;  /* OSRsemGO.c    */

void     OS_CreateRSema     (OS_RSEMA * pRSema)                  OS_SECTION_ATTRIBUTE;  /* OSRsem.c      */
void     OS_DeleteRSema     (OS_RSEMA * pRSema)                  OS_SECTION_ATTRIBUTE;  /* OSDelRS.c     */
...
void    OS_CreateCSema   (OS_CSEMA * pCSema, OS_UINT InitValue) OS_SECTION_ATTRIBUTE;  /* OSCSEM.c             */
void    OS_DeleteCSema   (OS_CSEMA * pCSema)                    OS_SECTION_ATTRIBUTE;  /* OSDELCS.c            */
int     OS_GetCSemaValue (OS_CONST_PTR OS_CSEMA *pCSema)        OS_SECTION_ATTRIBUTE;  /* OSCSEMGV.c           */
OS_U8   OS_SetCSemaValue (OS_CSEMA * pCSema, OS_UINT Value)     OS_SECTION_ATTRIBUTE;  /* OS_SetCSemaValue.c   */
void    OS_SignalCSema   (OS_CSEMA * pCSema)                    OS_SECTION_ATTRIBUTE;  /* OSCSEM.c             */
void    OS_SignalCSemaMax(OS_CSEMA * pCSema, OS_UINT MaxValue)  OS_SECTION_ATTRIBUTE;  /* OS_CSEMA_SignalMax.c */
void    OS_WaitCSema     (OS_CSEMA * pCSema)                    OS_SECTION_ATTRIBUTE;  /* OSCSEM.c             */
OS_BOOL OS_WaitCSemaTimed(OS_CSEMA * pCSema, OS_TIME TimeOut)   OS_SECTION_ATTRIBUTE;  /* OSCSEMT.c            */
OS_BOOL OS_CSemaRequest  (OS_CSEMA * pCSema)                    OS_SECTION_ATTRIBUTE;  /* OSCSEMRQ.c           */
...
void OS_CreateMB         (OS_MAILBOX * pMB, OS_U16 sizeofMsg, OS_UINT maxnofMsg, void* Buffer) OS_SECTION_ATTRIBUTE;  /* initialize mailbox */
void OS_ClearMB          (OS_MAILBOX * pMB)                                                    OS_SECTION_ATTRIBUTE;
void OS_PutMail          (OS_MAILBOX * pMB, OS_CONST_PTR void* pMail)                          OS_SECTION_ATTRIBUTE;
char OS_PutMailCond      (OS_MAILBOX * pMB, OS_CONST_PTR void* pMail)                          OS_SECTION_ATTRIBUTE;
void OS_PutMailFront     (OS_MAILBOX * pMB, OS_CONST_PTR void* pMail)                          OS_SECTION_ATTRIBUTE;
char OS_PutMailFrontCond (OS_MAILBOX * pMB, OS_CONST_PTR void* pMail)                          OS_SECTION_ATTRIBUTE;
void OS_PutMail1         (OS_MAILBOX * pMB, OS_CONST_PTR char* pMail)                          OS_SECTION_ATTRIBUTE;
char OS_PutMailCond1     (OS_MAILBOX * pMB, OS_CONST_PTR char* pMail)                          OS_SECTION_ATTRIBUTE;
void OS_PutMailFront1    (OS_MAILBOX * pMB, OS_CONST_PTR char* pMail)                          OS_SECTION_ATTRIBUTE;
char OS_PutMailFrontCond1(OS_MAILBOX * pMB, OS_CONST_PTR char* pMail)                          OS_SECTION_ATTRIBUTE;
void OS_GetMail          (OS_MAILBOX * pMB, void* pDest)                                       OS_SECTION_ATTRIBUTE;
char OS_GetMailCond      (OS_MAILBOX * pMB, void* pDest)                                       OS_SECTION_ATTRIBUTE;
void OS_GetMail1         (OS_MAILBOX * pMB, char* pDest)                                       OS_SECTION_ATTRIBUTE;
char OS_GetMailCond1     (OS_MAILBOX * pMB, char* pDest)                                       OS_SECTION_ATTRIBUTE;
char OS_GetMailTimed     (OS_MAILBOX * pMB, void* pDest, OS_TIME Timeout)                      OS_SECTION_ATTRIBUTE;
void OS_DeleteMB         (OS_MAILBOX * pMB)                                                    OS_SECTION_ATTRIBUTE;
char OS_PeekMail         (OS_CONST_PTR OS_MAILBOX * pMB, void* pDest)                          OS_SECTION_ATTRIBUTE;
void OS_WaitMail         (OS_MAILBOX * pMB)                                                    OS_SECTION_ATTRIBUTE;
char OS_WaitMailTimed    (OS_MAILBOX * pMB, OS_TIME Timeout)                                   OS_SECTION_ATTRIBUTE;
void OS_Mail_GetPtr      (OS_MAILBOX *pMB, void **ppDest)                                      OS_SECTION_ATTRIBUTE;
char OS_Mail_GetPtrCond  (OS_MAILBOX *pMB, void **ppDest)                                      OS_SECTION_ATTRIBUTE;
void OS_Mail_Purge       (OS_MAILBOX *pMB)                                                     OS_SECTION_ATTRIBUTE;

#if (OS_DEBUG == 0)
  #define OS_GetMessageCnt(pMB) (*(pMB)).nofMsg
#else
  OS_UINT OS_GetMessageCnt(const OS_MAILBOX *pMB) OS_SECTION_ATTRIBUTE;  /* get no. of available Messages */
#endif
...
void    OS_Q_Clear         (OS_Q* pQ)                                                                       OS_SECTION_ATTRIBUTE;  /* OSQCL.c  */
void    OS_Q_Delete        (OS_Q* pQ)                                                                       OS_SECTION_ATTRIBUTE;  /* OSQDEL.c */
int     OS_Q_GetMessageCnt (OS_CONST_PTR OS_Q* pQ)                                                          OS_SECTION_ATTRIBUTE;  /* OSQGMC.c */
int     OS_Q_Put           (OS_Q* pQ, OS_CONST_PTR void* pSrc, OS_UINT Size)                                OS_SECTION_ATTRIBUTE;
void    OS_Q_PutBlocked    (OS_Q* pQ, OS_CONST_PTR void* pSrc, OS_UINT Size)                                OS_SECTION_ATTRIBUTE;
char    OS_Q_PutTimed      (OS_Q* pQ, OS_CONST_PTR void* pSrc, OS_UINT Size, OS_TIME Timeout)               OS_SECTION_ATTRIBUTE;
int     OS_Q_PutEx         (OS_Q* pQ, OS_CONST_PTR OS_Q_SRCLIST* pSrcList, OS_UINT NumSrc)                  OS_SECTION_ATTRIBUTE;
void    OS_Q_PutBlockedEx  (OS_Q* pQ, OS_CONST_PTR OS_Q_SRCLIST* pSrcList, OS_UINT NumSrc)                  OS_SECTION_ATTRIBUTE;
char    OS_Q_PutTimedEx    (OS_Q* pQ, OS_CONST_PTR OS_Q_SRCLIST* pSrcList, OS_UINT NumSrc, OS_TIME Timeout) OS_SECTION_ATTRIBUTE;
int     OS_Q_GetPtr        (OS_Q* pQ, void**ppData)                                                         OS_SECTION_ATTRIBUTE;
int     OS_Q_GetPtrCond    (OS_Q* pQ, void**ppData)                                                         OS_SECTION_ATTRIBUTE; /* OSQGPC.c */
int     OS_Q_GetPtrTimed   (OS_Q* pQ, void**ppData, OS_TIME Timeout)                                        OS_SECTION_ATTRIBUTE; /* OSQGPT.c */
void    OS_Q_Purge         (OS_Q* pQ)                                                                       OS_SECTION_ATTRIBUTE;
OS_BOOL OS_Q_IsInUse       (OS_CONST_PTR OS_Q* pQ)                                                          OS_SECTION_ATTRIBUTE;
int     OS_Q_GetMessageSize(OS_CONST_PTR OS_Q* pQ)                                                          OS_SECTION_ATTRIBUTE;
int     OS_Q_PeekPtr       (OS_CONST_PTR OS_Q* pQ, void**ppData)                                            OS_SECTION_ATTRIBUTE;
...
OS_TASK_EVENT OS_ClearEvents         (OS_TASK * pTask)                          OS_SECTION_ATTRIBUTE;  /* OSEVENCL.c  */
OS_TASK_EVENT OS_GetEventsOccurred   (OS_CONST_PTR OS_TASK * pTask)             OS_SECTION_ATTRIBUTE;  /* OSEVENGE.c  */
void          OS_SignalEvent         (OS_TASK_EVENT Event, OS_TASK * pTask)     OS_SECTION_ATTRIBUTE;  /* OSENENS.c   */
OS_TASK_EVENT OS_WaitEvent           (OS_TASK_EVENT EventMask)                  OS_SECTION_ATTRIBUTE;  /* OSEVENW.c   */
OS_TASK_EVENT OS_WaitEventTimed      (OS_TASK_EVENT EventMask, OS_TIME TimeOut) OS_SECTION_ATTRIBUTE;  /* OSEVENT.c   */
OS_TASK_EVENT OS_WaitSingleEvent     (OS_TASK_EVENT EventMask)                  OS_SECTION_ATTRIBUTE;  /* OSEVENWS.c  */
OS_TASK_EVENT OS_WaitSingleEventTimed(OS_TASK_EVENT EventMask, OS_TIME TimeOut) OS_SECTION_ATTRIBUTE;  /* OSEVENWST.c */
...
void    OS_RetriggerTimer(OS_TIMER * pTimer)                                             OS_SECTION_ATTRIBUTE;
void    OS_StartTimer    (OS_TIMER * pTimer)                                             OS_SECTION_ATTRIBUTE;
void    OS_StopTimer     (OS_TIMER * pTimer)                                             OS_SECTION_ATTRIBUTE;
void    OS_TriggerTimer  (OS_TIMER * pTimer)                                             OS_SECTION_ATTRIBUTE;
void    OS_DeleteTimer   (OS_TIMER * pTimer)                                             OS_SECTION_ATTRIBUTE;  /* OSTIMED.c  */
OS_TIME OS_GetTimerPeriod(OS_CONST_PTR OS_TIMER * pTimer)                                OS_SECTION_ATTRIBUTE;  /* OSTIMEGP.c */
OS_BOOL OS_GetTimerStatus(OS_CONST_PTR OS_TIMER * pTimer)                                OS_SECTION_ATTRIBUTE;  /* OSTIMEGS.c */
OS_TIME OS_GetTimerValue (OS_CONST_PTR OS_TIMER * pTimer)                                OS_SECTION_ATTRIBUTE;  /* OSTIMEGV.c */
void    OS_SetTimerPeriod(OS_TIMER * pTimer, OS_TIME Period)                             OS_SECTION_ATTRIBUTE;  /* OSTIMES.c  */
...
OS_BOOL             OS_EVENT_Get         (OS_CONST_PTR OS_EVENT* pEvent)                   OS_SECTION_ATTRIBUTE;
OS_EVENT_RESET_MODE OS_EVENT_GetResetMode(const OS_EVENT* pEvent)                          OS_SECTION_ATTRIBUTE;
void                OS_EVENT_Pulse       (OS_EVENT* pEvent)                                OS_SECTION_ATTRIBUTE;
void                OS_EVENT_Reset       (OS_EVENT* pEvent)                                OS_SECTION_ATTRIBUTE;
void                OS_EVENT_Set         (OS_EVENT* pEvent)                                OS_SECTION_ATTRIBUTE;
void                OS_EVENT_SetResetMode(OS_EVENT* pEvent, OS_EVENT_RESET_MODE ResetMode) OS_SECTION_ATTRIBUTE;
void                OS_EVENT_Wait        (OS_EVENT* pEvent)                                OS_SECTION_ATTRIBUTE;
char                OS_EVENT_WaitTimed   (OS_EVENT* pEvent, OS_TIME Timeout)               OS_SECTION_ATTRIBUTE;
...
void    OS_Timing_Start(OS_TIMING* pCycle)              OS_SECTION_ATTRIBUTE;
void    OS_Timing_End  (OS_TIMING* pCycle)              OS_SECTION_ATTRIBUTE;
OS_U32  OS_Timing_Getus(OS_CONST_PTR OS_TIMING* pCycle) OS_SECTION_ATTRIBUTE;
#define OS_Timing_GetCycles(pPara) (*(pPara))

OS_TIME OS_GetNumIdleTicks  (void)                                OS_SECTION_ATTRIBUTE;  /* OS_GetNumIdleTicks.c */
void    OS_AdjustTime       (OS_TIME Time)                        OS_SECTION_ATTRIBUTE;  /* OS_AdjustTime.c      */
void    OS_StartTicklessMode(OS_TIME Time, voidRoutine *Callback) OS_SECTION_ATTRIBUTE;  /* OS_Tickless.c        */
void    OS_StopTicklessMode (void)                                OS_SECTION_ATTRIBUTE;  /* OS_Tickless.c        */
...
void    OS_POWER_UsageInc(OS_UINT Index) OS_SECTION_ATTRIBUTE;
void    OS_POWER_UsageDec(OS_UINT Index) OS_SECTION_ATTRIBUTE;
OS_UINT OS_POWER_GetMask(void)           OS_SECTION_ATTRIBUTE;
#endif
...
**********************************************************************
*/
void OS_SPINLOCK_Create   (OS_SPINLOCK*    pSpinlock)             OS_SECTION_ATTRIBUTE;  /* OS_Spinlock.c                     */
void OS_SPINLOCK_Lock     (OS_SPINLOCK*    pSpinlock)             OS_SECTION_ATTRIBUTE;  /* optional, implemented in OSSrcCPU */
void OS_SPINLOCK_Unlock   (OS_SPINLOCK*    pSpinlock)             OS_SECTION_ATTRIBUTE;  /* OS_Spinlock.c                     */
void OS_SPINLOCK_SW_Create(OS_SPINLOCK_SW* pSpinlock)             OS_SECTION_ATTRIBUTE;  /* OS_Spinlock.c                     */
void OS_SPINLOCK_SW_Lock  (OS_SPINLOCK_SW* pSpinlock, OS_UINT Id) OS_SECTION_ATTRIBUTE;  /* OS_Spinlock.c                     */
void OS_SPINLOCK_SW_Unlock(OS_SPINLOCK_SW* pSpinlock, OS_UINT Id) OS_SECTION_ATTRIBUTE;  /* OS_Spinlock.c                     */
...
*/
  ...
  void OS_TraceEnableAll       (void)                          OS_SECTION_ATTRIBUTE;  /* OSTrace1.c */
  void OS_TraceDisableAll      (void)                          OS_SECTION_ATTRIBUTE;  /* OSTrace1.c */
  void OS_TraceEnableId        (OS_U8 id)                      OS_SECTION_ATTRIBUTE;  /* OSTrace1.c */
  void OS_TraceDisableId       (OS_U8 id)                      OS_SECTION_ATTRIBUTE;  /* OSTrace1.c */
  void OS_TraceEnableFilterId  (OS_U8 FilterIndex, OS_U8 id)   OS_SECTION_ATTRIBUTE;  /* OSTrace1.c */
  void OS_TraceDisableFilterId (OS_U8 FilterIndex, OS_U8 id)   OS_SECTION_ATTRIBUTE;  /* OSTrace1.c */
  ...
  typedef struct {
    ...
    void (*pfTraceStoreEvent)      (int nParam, OS_U16 EventID, ...);  /*lint !e579 Suppress warning about parameter preceding ellipsis has invalid type */
  ...
  void OS_PTraceVoid           (OS_U16 id)                                           OS_SECTION_ATTRIBUTE;
  void OS_PTracePtr            (OS_U16 id, void *p)                                  OS_SECTION_ATTRIBUTE;
  void OS_PTraceData           (OS_U16 id, OS_U32 v)                                 OS_SECTION_ATTRIBUTE;
  void OS_PTraceDataData       (OS_U16 id, OS_U32 v1, OS_U32 v2)                     OS_SECTION_ATTRIBUTE;
  void OS_PTracePtrData        (OS_U16 id, void *p, OS_U32 v)                        OS_SECTION_ATTRIBUTE;
  void OS_PTracePtrDataData    (OS_U16 id, void *p, OS_U32 v1, OS_U32 v2)            OS_SECTION_ATTRIBUTE;
  void OS_PTracePtrDataDataData(OS_U16 id, void *p, OS_U32 v1, OS_U32 v2, OS_U32 v3) OS_SECTION_ATTRIBUTE;
  void OS_PTracePtrStr         (OS_U16 id, void *p, const char *c)                   OS_SECTION_ATTRIBUTE;
  void OS_PTraceSaveSymbol     (void* p, const char *n)                              OS_SECTION_ATTRIBUTE;
  void OS_PTraceSaveObjectData (void* p, OS_U32 d)                                   OS_SECTION_ATTRIBUTE;
  void OS_PTraceInit           (const OS_PTRACE_FUNC *p)                             OS_SECTION_ATTRIBUTE;
...
OS_U32 OS_ConvertCycles2us(OS_U32 Cycles)   OS_SECTION_ATTRIBUTE;
...
OS_INTERWORK void  OS_StartASM     (void) OS_SECTION_ATTRIBUTE;   /* RTOS ASM part   */
OS_INTERWORK int   OS_SwitchFromInt(void) OS_SECTION_ATTRIBUTE;   /* RTOS ASM part   */

void            OS_InitKern         (void)                                               OS_SECTION_ATTRIBUTE;  /* OSKern.c */
OS_U8           OS_OnTx             (void)                                               OS_SECTION_ATTRIBUTE;  /* OSComRxTx1.c, Returns 1 on last byte */
OS_INT          OS_COM_GetNextChar  (void)                                               OS_SECTION_ATTRIBUTE;  /* OSComRxTx1.c, Returns the character to send, or -1  */
void            OS_OnRx             (OS_U8 Data)                                         OS_SECTION_ATTRIBUTE;  /* OSComRxTx1.c */
void            OS_Send1            (OS_U8 c)                                            OS_SECTION_ATTRIBUTE;  /* RTOSInit.c */
void            OS_SendString       (OS_ROM_DATA const char* s)                          OS_SECTION_ATTRIBUTE;  /* OSSend.c */
void            OS_SendStringFast   (OS_ROM_DATA const char *s)                          OS_SECTION_ATTRIBUTE;  /* No official API function */
OS_RX_CALLBACK* OS_SetRxCallback    (OS_RX_CALLBACK* cb)                                 OS_SECTION_ATTRIBUTE;
OS_U8           OS_EvaPacketEx      (const OS_U8 pSrc[], OS_U16 SrcLen, OS_U8** pReturn) OS_SECTION_ATTRIBUTE;  /* OS_EvaPacketEx.c */
void            OS_COM_ClearTxActive(void)                                               OS_SECTION_ATTRIBUTE;  /* OSCom2.c */

#define OS_Start()  OS_Running = 1u; OS_StartASM()

/********** Additional functions prototypes for Percepio Trace *******/
OS_U32 OS_GetPClkTimer(void) OS_SECTION_ATTRIBUTE;
OS_U32 OS_GetTickFreq (void) OS_SECTION_ATTRIBUTE;
...
*/
#ifndef OS_GetTime
  OS_TIME OS_GetTime(void) OS_SECTION_ATTRIBUTE;
#endif
#ifndef OS_GetTime32
  #if (OS_SIZEOF_INT == 4)
    #define OS_GetTime32() (OS_Global.Time)
  #else
    OS_I32 OS_GetTime32(void) OS_SECTION_ATTRIBUTE;
  #endif
#endif
  ...
  OS_U32 OS_GetTime_us     (void)                              OS_SECTION_ATTRIBUTE;  /* Routine for calculating current system time in usec */
  OS_U64 OS_GetTime_us64   (void)                              OS_SECTION_ATTRIBUTE;
#endif
...
void OS_TICK_HandleEx    (void)                                               OS_SECTION_ATTRIBUTE;  /* OS_TICK_HandleEx.c */
