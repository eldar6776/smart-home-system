...
#define USE_RX_TASK       1  // 0: Packets are read in ISR, 1: Packets are read in a task of its own.
...
/* Global data */
static OS_STACKPTR int _Stack[TASK_STACK_SIZE_IP_TASK];  // Task stack
static OS_TASK         _TCB;                     // Task-control-block

#if USE_RX_TASK
static OS_STACKPTR int _IPRxStack[TASK_STACK_SIZE_IP_RX_TASK];  // Task stack
static OS_TASK         _IPRxTCB;                        // Task-control-block
#endif
...
void MainTask(void) {
  ...
  // Start TCP/IP task
  //
  OS_CREATETASK(&_TCB,     "IP_Task",   IP_Task,   TASKPRIO_IPMAIN, _Stack);
#if USE_RX_TASK
  OS_CREATETASK(&_IPRxTCB, "IP_RxTask", IP_RxTask, TASKPRIO_IPRX,   _IPRxStack);  // Start the IP_RxTask, optional.
#endif
  //
  // Start VNC server
  //
