...
Purpose : Sample program for Modbus slave using embOS.
          ...
          The sample setups a Modbus RTU slave via a serial
...
void MainTask(void) {
  ...
  BSP_ClrLED(1);
  //
  // Start Modbus slave using serial RTU protocol.
  //
  MB_SLAVE_Init();
  MB_SLAVE_AddRTUChannel(&_MBChannel, &_MBConfig, &_SlaveAPI, &_IFaceAPI, SLAVE_ADDR, 0, UART_BAUDRATE, UART_NUM_DATA_BITS, UART_PARITY, UART_NUM_STOP_BITS, UART_PORT);  // Add a slave channel.
