...
static OS_STACKPTR int _aFTPStack[MAX_CONNECTIONS][3200/sizeof(int)];       // Stacks of the FTP server child tasks.
  ...
  while (1) {
    ...
    if (_ConnectCnt < MAX_CONNECTIONS) {
      for (i = 0; i < MAX_CONNECTIONS; i++) {
        ...
        if (r == 0) {
          OS_EnterRegion();
          OS_CREATETASK_EX(&_aFTPTCB[i], "FTPServer_Child", _FTPServerChildTask, TASK_PRIO_FTPS_CHILD, _aFTPStack[i], (void*)hSock);
          _AddToConnectCnt(1);
          OS_LeaveRegion();
          break;
        }
