...
Purpose : Sample program for Modbus master using embOS.
          ...
          The sample connects to a Modbus ASCII slave via a serial
...
static void _ModbusHPMasterTask(void) {
  ...
  MB_MASTER_Init();
  MB_MASTER_AddASCIIChannel(&_MBChannel, &_MBConfig, &_IFaceAPI, TIMEOUT, SLAVE_ADDR, UART_BAUDRATE, UART_NUM_DATA_BITS, UART_PARITY, UART_NUM_STOP_BITS, UART_PORT);
  do {
    State ^= 1;
    LOCK();
