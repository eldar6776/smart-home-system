...
static void _DeInitTimerRTU  (void);
static void _DeInitTimerASCII(void);
...
static void  _SendByte(MB_IFACE_CONFIG_UART *pConfig, const U8  Data) {
  ...
  if (_hCom != INVALID_HANDLE_VALUE) {
    ...
    // instead let's start from the timer thread to avoid reentrency
    //
    if (_WriteMsg == 0) {
      _WriteMsg = 1;
    } else {
...
static int _Send(MB_IFACE_CONFIG_UART *pConfig, const U8 *pData, U32 NumBytes) {
  ...
  if (_hCom != INVALID_HANDLE_VALUE) {
    //
    // Windows is not a realtime system, thus it is not possible to have the exact 3,5 silence period
    //
...
*/
void CALLBACK _TimeProcASCII(UINT uID, UINT uMsg, DWORD dwUser, DWORD dw1, DWORD dw2) {
  ...
  if (_hCom != INVALID_HANDLE_VALUE) {
    ...
    for (i = 0; i < nofBytes; i++) {
      MB_OnRx(&_MBChannel, _RxBuffer[i]);
    }
    //
    ...
    if (_WriteMsg) {
      ...
      while (MB_OnTx(&_MBChannel) == 0); // will call _SendByte for each character
      _WriteMsg = 0;
    }
  }
...
*
* Function description
*   Deinitialize timer for UART ASCII interface.
*/
static void _DeInitTimerASCII(void) {
...
*
* Function description
*   Deinitialize timer for UART RTU interface.
*/
static void _DeInitTimerRTU(void) {
  // Not used
}
