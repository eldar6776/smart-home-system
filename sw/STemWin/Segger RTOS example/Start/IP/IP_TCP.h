...
#if IP_SUPPORT_IPV6
typedef struct TCP_IPV6_HEADER {
...
//
int      IP_TCP_PCB_bind       (TCPCB *, struct sockaddr * pSockAddr);
int      IP_TCP_PCB_connect    (TCPCB *, struct sockaddr * pSockAddr);
TCPCB *  IP_TCP_PCB_lookup     (void * pFAddr, U16 FPort, void * pLAddr, U16 LPort, int Flags);
...
void     IP_TCP_Drop              (TCPCB * pTCPCB, int);
...
typedef struct {
  ...
  U32 RxWindowSize;
  U16 NextFreePort;
  TCP_FUNCTIONS * pTCPFunc;
} IP_TCP_GLOBAL;

EXTERN IP_TCP_GLOBAL IP_TCP_Global;
EXTERN TCP_FUNCTIONS IP_TCP_Func;
