...
typedef struct IPV6_PREFIX_LIST {
  union {
    IP_DLIST_HEAD   Head;
...
*/
typedef struct IPV6_ROUTER_ADDR {
  IP_DLIST_ITEM Link;
  IPV6_ADDR     IPAddr;
...
typedef struct IPV6_DEF_ROUTER_LIST {
  union {
    IP_DLIST_HEAD     Head;
...
*/
typedef struct IPV6_DNS_ADDR {
  IP_DLIST_ITEM Link;
  IPV6_ADDR     IPAddr;
} IPV6_DNS_SERVER_ADDR;

typedef struct IPV6_DNS_SERVER_LIST {
  union {
    IP_DLIST_HEAD     Head;
    IPV6_ROUTER_ADDR* pFirstAddr;
  } List;
...
typedef struct IPV6_MULTICAST_ADDR {
  IP_DLIST_ITEM Link;
  IPV6_ADDR     IPAddr;  
  unsigned      tDelay;
} IPV6_MULTICAST_ADDR;

typedef struct IPV6_LINK_DESC {
  struct {
    union {
      IP_DLIST_HEAD     Head;
...
int              IP_ICMPV6_NDP_SendNeighborSolicitationPacket  (U8 IFaceId, IPV6_ADDR* pDestIP);

//
// Multicast Listener Discovery (MLD) protocol
//
void             IP_ICMPV6_MLD_ConfIFace                       (U8 IFaceId);
void             IP_ICMPV6_MLD_InitModule                      (void);
void             IP_ICMPV6_MLD_Timer                           (void);
void             IP_ICMPV6_MLD_OnRx                            (U8 Type, IP_PACKET* pPacket);
void             IP_ICMPV6_MLD_Query_OnRx                      (IP_PACKET* pPacket);
int              IP_ICMPV6_MLD_GenAndAddSolicitedNodeAddr      (U8 IFaceId, IPV6_ADDR* pIPv6Addr);
int              IP_ICMPV6_MLD_GenAndAddSolicitedNodeAddrEx    (U8 IFaceId, IPV6_ADDR* pIPv6Addr, unsigned UpdateFilters);
...
int              IP_ICMPV6_SendParaProbMsg                     (U8 IFaceId, IP_PACKET *pProbPacket, U8 Code, U32 Offset);
...
IPV6_ADDR_DESC*  IP_IPV6_GenUnicastAddr                        (U8 IFaceId, U32 ValidLifetime, U32 PreferredLifetime, const IPV6_PREFIX* pPrefix, const IPV6_ROUTER_ADDR* pRouterAddr);
...
IFACE*           IP_IPV6_GetIFaceForHost                       (U16 ProtoFlags, IPV6_ADDR* pAddr); 
void             IP_IPV6_SetLinkLocalUnicastAddr               (U8 IFaceId);
